...
- [6. Manual Test Cases](#6-manual-test-cases)
  ...
  - [6.8 Persona Exploration](#68-persona-exploration)
...
| Persona usernames   | `problem_user`, `performance_glitch_user`, `error_user`, `visual_user` |
| First name          | `Katia`                                                                |
| Last name           | `Tester`                                                               |
| Postal code         | `12345`                                                                |
...
The automation suite is implemented incrementally with Playwright and TypeScript.
...
- `npm test` - standard regression suite; excludes tests tagged `@persona`.
- `npm run test:personas` - focused persona-specific known-issue checks.
- `npm run test:all` - both suites across all configured browsers.

Persona-risk checks use Playwright `test.fail()` for documented, reproducible known issues. An unexpected pass makes the run fail, drawing attention to a possible product fix.

GitHub Actions runs `npm test` on push and pull request events and uploads the Playwright HTML report as a workflow artifact.
...
| Sidebar menu navigation                                           | Covered              | Completed                            |
| Logout flow                                                       | Covered              | Completed                            |
| Focused persona risk checks                                       | Covered              | Completed as known expected failures |
...
2. Locate the `About` link.

#### Expected Result

- The `About` link is visible and points to the Sauce Labs domain.
- Automated regression validates the link destination without depending on the external site's availability.
...
Two stable, high-value scenarios are automated as known expected failures:

- Product Details routing does not preserve the selected product identity (`BUG-INV-001`);
- Last Name input blocks checkout (`BUG-CHK1-001`).

Image, sorting, and Add/Remove observations remain exploratory because they are either lower value for the selected suite or require a more deterministic reproduction path. The complete `standard_user` regression suite is not duplicated for `problem_user`.
...
Keep this charter exploratory. There is no agreed product performance SLA, so a pass/fail timing assertion would be arbitrary and potentially flaky. When navigation tests are added in the future, they should assert page readiness after navigation rather than URL change alone.
...
- sorting triggers an alert and does not change the product order (`BUG-INV-004`);
- Finish does not complete the order (`BUG-CHK2-001`).

Missing Product Details description, Last Name behavior, incomplete-data continuation, and stale Reset App State controls remain exploratory observations. Inconsistent Add and Remove behavior remains exploratory until a deterministic reproduction path is established.
...
- Inventory price does not match Product Details for the same product (`BUG-INV-006`);
- sorting does not preserve the product-image mapping (`BUG-INV-007`).

Price stability across further pages, currency formatting, intermittent monetary rounding, and broad layout observations remain exploratory. The complete `standard_user` regression suite is not duplicated for `visual_user`.
...
| TC-MENU-004                 | Sidebar Menu                  | About link destination points to the `saucelabs.com` domain                                                                                  |
...
All 43 planned functional regression test cases are automated.
Exploratory charters remain partially automated where appropriate.
