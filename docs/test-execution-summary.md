...
- `First Name` field was left empty.
...
- `First Name` field remained empty.
- `First Name` placeholder and underline were highlighted in red.
- A red validation icon was displayed next to the `First Name` field.
...
- The page prevented navigation to Checkout Overview when the required `First Name` field was empty.
- The entered values in the other fields were preserved after validation failed.
- Red validation icons were displayed next to all three form fields, although the error message referred only to the missing `First Name`.
- This may make it appear that all fields are invalid, even though `Last Name` and `Zip/Postal Code` contain valid values.
...
- User was redirected to `/checkout-step-two.html`.
- Page title `Checkout: Overview` was displayed.
- Exactly one product was displayed.
- Product name `Sauce Labs Backpack` was displayed.
- Product quantity was `1`.
- Product description was displayed.
- Product price `$29.99` was displayed.
- Cart badge still displayed `1`.
...
- The selected product was preserved throughout the checkout flow.
- Product name, quantity, description, and price matched the product added on the Inventory Page.
- The product description begins with code-like demo text: `carry.allTheThings()`.
- Similar code-like content is used elsewhere in SauceDemo and is treated as demo data rather than a functional defect.
...
- The sidebar menu closed successfully.
- `All Items` was no longer visible.
- `About` was no longer visible.
- `Logout` was no longer visible.
- `Reset App State` was no longer visible.
- The `Open Menu` button was available again.
- User remained on `/inventory.html`.
- Page title `Products` remained visible.
...
- User opened the sidebar menu.
- User clicked `About`.
- User was redirected away from SauceDemo.
- Final URL was `https://saucelabs.com/`.
- The Sauce Labs website loaded successfully.
- Sauce Labs branding and page content were visible.
- No browser error page was displayed.
